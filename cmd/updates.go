...
func muFindAndSync() {
	...
	if cleanup {
		...
		if spitYamls {
			...
			err := ioutil.WriteFile(fmt.Sprintf("./minima_obs_%v.yaml", time.Now()), byteChunk, 0644)
			if err != nil {
				log.Fatalf("There is an error: %v", err)
			}
			os.Exit(3)
		...
		if justSearch {
			for _, value := range updateList {
				fmt.Printf("INCIDENT: %v   PACKAGES: %v\n", value.IncidentNumber, value.SRCRPMS )
			}
			os.Exit(3)
		} 
...
func ArchMage(repo HTTPRepoConfig) (HTTPRepoConfig, error) {
	archRegister := []string{"x86_64", "i586", "i686", "aarch64", "aarch64_ilp32", "ppc64le", "s390x"}
